...
def repositories(operation_repository: JsonOperationRepository = Depends(get_json_operation_repository),
    ...
    return {"operation": operation_repository, "user": user_repository}

repositoriesDep = Annotated[dict, Depends(repositories)]

@router.post("/create", response_model=OperationOutDetail)
def create_operation(operation_to_create: OperationInput, repositories: repositoriesDep):

    try:
        operation_factory = StandardOperationFactory()
        operation_service = OperationService(
            repositories["operation"], repositories["user"])
        ...
        created_operation = operation_service.create_operation(
...
def get_all_available_operations(repositories: repositoriesDep):
...
def get_detailed_operation_by_id(operation_id: str, repositories: repositoriesDep):
    try:
...
def get_all_operation_by_user_id(user_id, repositories: repositoriesDep):
...
@router.post("/delete")
def delete_operation(operation_id: str, user_id: str, repositories: repositoriesDep,
